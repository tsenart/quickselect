...
func insertionSort_func(data Interface, a, b int) {
	for i := a + 1; i < b; i++ {
...
func siftDown_func(data Interface, lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func heapSort_func(data Interface, a, b int) {
	first := a
	lo := 0
	hi := b - a
...
func pdqsort_func(data Interface, a, b, limit int) {
...
func partition_func(data Interface, a, b, pivot int) (newpivot int, alreadyPartitioned bool) {
	data.Swap(a, pivot)
...
func partitionEqual_func(data Interface, a, b, pivot int) (newpivot int) {
	data.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
func partialInsertionSort_func(data Interface, a, b int) bool {
	const (
...
func breakPatterns_func(data Interface, a, b int) {
	length := b - a
	if length >= 8 {
...
func choosePivot_func(data Interface, a, b int) (pivot int, hint sortedHint) {
	const (
		shortestNinther = 50
		maxSwaps        = 4 * 3
	)
...
func order2_func(data Interface, a, b int, swaps *int) (int, int) {
	if data.Less(b, a) {
		*swaps++
		return b, a
	}
...
func median_func(data Interface, a, b, c int, swaps *int) int {
...
func medianAdjacent_func(data Interface, a int, swaps *int) int {
	return median_func(data, a-1, a, a+1, swaps)
}

func reverseRange_func(data Interface, a, b int) {
	i := a
	j := b - 1
	for i < j {
		data.Swap(i, j)
...
func swapRange_func(data Interface, a, b, n int) {
	for i := 0; i < n; i++ {
		data.Swap(a+i, b+i)
	}
}

func stable_func(data Interface, n int) {
...
func symMerge_func(data Interface, a, m, b int) {
...
func rotate_func(data Interface, a, m, b int) {
	i := m - a
	j := b - m
